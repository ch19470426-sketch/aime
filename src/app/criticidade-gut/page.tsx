...
const TIPOS = ['Gravidade', 'Urgência', 'Abrangência', 'Exposição']
const formInicial: Item = { tipo_parametro: 'Gravidade', descricao: '', peso: 1, percentual_calculo: null, ativo: true }
...
export default function CriticidadeGutPage() {
  ...
  const [tipoFiltro, setTipoFiltro] = useState('Gravidade')
  ...
  function novo() {
    setForm({...formInicial, tipo_parametro: tipoFiltro})
    setModoEdicao(false)
    setMsg(''); setErro('')
  }
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      if (modoEdicao && form.id) {
        const { error } = await supabase.from('criticidade_gut')
          .update({ tipo_parametro: form.tipo_parametro, descricao: form.descricao,
            peso: form.peso, percentual_calculo: form.percentual_calculo, ativo: form.ativo })
          .eq('id', form.id)
        if (error) { setErro(error.message); return }
        setMsg('Atualizado com sucesso!')
      } else {
        const { error } = await supabase.from('criticidade_gut')
          .insert({ tipo_parametro: form.tipo_parametro, descricao: form.descricao,
            peso: form.peso, percentual_calculo: form.percentual_calculo, ativo: form.ativo })
        if (error) { setErro(error.message); return }
        setMsg('Incluído com sucesso!')
        novo()
      }
      await carregar()
  ...
  const itensFiltrados = itens.filter(i => i.tipo_parametro === tipoFiltro)
  // Percentual do tipo atual
  const pctAtual = itens.find(i => i.tipo_parametro === tipoFiltro && i.percentual_calculo != null)?.percentual_calculo

  return (
    <div style={{ backgroundColor:'#E8EEF7', minHeight:'100vh', padding:'16px' }}>
      <div style={{ maxWidth:'960px', margin:'0 auto', backgroundColor:'white', borderRadius:'16px', boxShadow:'0 4px 24px rgba(0,0,0,0.12)', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', minHeight:'500px' }}>
          ...
          <div style={{ width:'280px', borderRight:'2px solid #1E3A8A', flexShrink:0 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'0', borderBottom:'2px solid #1E3A8A' }}>
              {TIPOS.map(t => (
                <button key={t} onClick={() => { setTipoFiltro(t); novo() }}
                  style={{ flex:'1 1 auto', padding:'6px 4px', border:'none', cursor:'pointer', fontSize:'10px', fontWeight:700,
                    backgroundColor: tipoFiltro===t ? '#1E3A8A' : 'white',
                    color: tipoFiltro===t ? 'white' : '#1E3A8A' }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div style={{ flex:1, padding:'20px', backgroundColor:'#F8FAFC' }}>
              ...
              <div style={blocoStyle}>
                <div style={blocoHeaderStyle}>
                  <span style={{ color:'white', fontWeight:700, fontSize:'12px' }}>
                    {modoEdicao ? `Editar — ${form.tipo_parametro}` : `Novo — ${form.tipo_parametro}`}
                  </span>
                </div>
                <div style={blocoBodyStyle}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
                    ...
                    <div>
                      <label style={labelStyle}>% no Cálculo GR <span style={{ fontWeight:400, color:'#6B7280' }}>(somente 1° do tipo)</span></label>
            ...
            <div style={{ backgroundColor:'#EBF1FF', borderRadius:'8px', padding:'12px', fontSize:'11px', color:'#1E3A8A', lineHeight:1.7 }}>
              ...
              Padrão atual: Gravidade 40% · Urgência 20% · Abrangência 30% · Exposição 10%
            </div>
          </div>
        </div>
      </div>
