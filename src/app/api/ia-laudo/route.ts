...
- Sem markdown, sem asteriscos, sem listas, apenas texto corrido em parágrafo único`
      ...
      const especialista: Record<string,string> = {
        '45': 'engenheiro especialista em inspeção de elevadores',
        '46': 'engenheiro especialista em segurança em instalações e serviços em eletricidade (NR-10)',
        '47': 'engenheiro especialista em segurança no trabalho em máquinas e equipamentos (NR-12)',
        '48': 'engenheiro especialista em segurança em caldeiras, vasos de pressão e tubulações (NR-13)',
      }
      const perfilVistoria = especialista[String(d.tipo_servico)] ?? 'engenheiro especialista em inspeção predial'
      prompt = `Você é um ${perfilVistoria}. Redija o texto do item 3.1 — Descrição da Vistoria Técnica — do laudo, descrevendo COMO a vistoria foi realizada.\n\nANOTAÇÕES DO INSPETOR (base principal do texto, use integralmente):\n${d.informacoes || '(sem anotações)'}\n\nDADOS COMPLEMENTARES:\n- Edificação/Estabelecimento: ${d.razao_social || ''}\n- Data de realização da vistoria (usar apenas se o inspetor mencionar nas anotações): ${d.data_vistoria || ''}\
- Nível de inspeção: ${d.nivel_inspecao || ''}\n- Sistemas COM não conformidades (cite apenas estes): ${d.sistemas || ''}\n- Não conformidades registradas: ${d.qtd_ncs ?? ''}\n- Inspetor: ${d.nome_inspetor || ''}\n\nINSTRUÇÕES:\n- Parta das anotações do inspetor e as reescreva em linguagem técnica formal, na terceira pessoa e no passado.\
- NÃO mencione datas, períodos ou horários — omita completamente qualquer referência temporal.\n- Descreva o método empregado, os sistemas percorridos e a forma de registro das não conformidades.\n- Não invente dados que não constem acima.\n- Mencione APENAS os sistemas listados acima — não cite sistemas não vistoriados.\n- Entre 400 e 800 caracteres, texto corrido em parágrafo único.\n- Sem título, sem prefixo numérico, sem markdown, sem asteriscos e sem listas.`
      ...
      if (ehNR) {
        ...
        prompt = `Você é um engenheiro especialista em segurança do trabalho e conformidade regulatória (NR-10, NR-12, NR-13), atuando como diagnóstico de instalações industriais. Com base nas não conformidades identificadas abaixo, redija recomendações técnicas GERAIS para a instalação como um todo — não por não conformidade individual. As recomendações devem abordar: 5.1 Manutenção preventiva dos equipamentos e sistemas; 5.2 Operação segura e procedimentos; 5.3 Condições físicas e infraestrutura; 5.4 Segurança do trabalho e conformidade com NRs; 5.5 Documentação e registros técnicos. Cada subitem deve ter até 3 recomendações. Cada recomendação: máximo 400 caracteres, até 3 frases, objetiva, sem justificativa, linguagem técnica, pontuação correta em português.\n\nNÃO CONFORMIDADES IDENTIFICADAS:\n${ncsTexto}\n\nResponda APENAS com 5 parágrafos separados por linha em branco (um por subitem 5.1 a 5.5), sem títulos ou prefixos.`
      } else {
