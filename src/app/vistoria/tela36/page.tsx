...
const TITULO_TELA: Record<string, string> = {
  ...
  '34': 'Vistoria Fachada', '35': 'Vistoria Elevador', '36': 'Vistoria Instalções Eléricas - NR-10',
...
function Tela31Inner() {
  ...
  async function gerarNcCp(foto: string) {
    // Telas NR: IA só é acionada se resultado = Não conforme
      ...
      <div style={S.page}>
        ...
        <div style={S.formBody}>
          ...
          <div style={S.block}>
            ...
            <div style={S.blockBody}>
              <Field label={resultado === 'Não conforme' ? 'Descrição da Não Conformidade (gerada por IA)' : 'Observações'}>
                <textarea style={{ ...S.input, ...S.textarea }}
                  value={nc}
                  maxLength={500}
                  readOnly={resultado === 'Conforme' || resultado === 'Não aplicável'}
                  onChange={e => setNc(e.target.value)}
                  placeholder={resultado === 'Não conforme' ? 'Gerado por IA após adicionar foto...' : resultado === 'Não verificado' ? 'Justifique o motivo...' : 'Selecione o resultado primeiro...'} />
              </Field>
            </div>
          </div>
