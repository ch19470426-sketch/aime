...
// ── Fallback GUT (usado se banco indisponível) ───────────────────────────────
    ...
    async function carregar() {
      ...
      try {
        ...
        if (cpfInspetor) {
          ...
          try {
            const rCC = await fetch(`/api/contato-cliente?cpf_inspetor=${cpfInspetor}&cnpjoucpf=${cnpjoucpf}&tipo_servico=${encodeURIComponent('33 Vistoria imóvel novo')}`)
        ...
        fetch('/api/criticidade-gut?tipo_servico=33%20Vistoria%20im%C3%B3vel%20novo')
          .then(r => r.json())
          .then(d => {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          fotoOffline = cv2.toDataURL('image/jpeg', 0.5)
        } catch {}
          ...
          <div style={S.block}>
            ...
            <div style={S.blockBody}>
              <div style={{ ...S.row, ...S.c2 }}>
                <Field label={cnpjoucpf.length === 11 ? "CPF" : "CNPJ"}><input style={S.input} value={fmtDoc(cnpjoucpf)} readOnly /></Field>
              ...
              <div style={{ ...S.row, ...S.c3 }}>
                ...
                <Field label={tagObrigatorio ? 'Tag / Nr série *' : 'Tag / Nr série'}>
                  <select style={S.input} value={tagNrSerie} onChange={e => {
                    ...
                    // finalidade vem de contato_cliente
                  }} disabled={!tipoAtivo}>
                    <option value="">Selecione...</option>
          ...
          <div style={S.block}>
            ...
            <div style={S.blockBody}>
              ...
              <div style={S.photoArea} onClick={() => {
                ...
                {fotoBase64 && <img src={fotoBase64} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain' }} />}
...
const S: Record<string, React.CSSProperties> = {
  ...
  photoArea:     { border: '1.5px dashed #c3d4f0', borderRadius: '5px', background: '#000', height: '280px', maxWidth: '380px', margin: '0 auto', position: 'relative', overflow: 'hidden', cursor: 'pointer', display: 'block', width: '100%' },
