    ...
    async function carregar() {
      ...
      try {
        ...
        if (cpfInspetor) {
          ...
          try {
            const resCC = await fetch(`/api/contato-cliente?cpf_inspetor=${cpfInspetor}&cnpjoucpf=${cnpjoucpf}&tipo_servico=${encodeURIComponent('31 Autovistoria')}`)
          ...
          try {
            const resCC = await fetch(`${SUPA_URL}/rest/v1/contato_cliente?cpf_inspetor=eq.${cpfInspetor}&cnpjoucpf=eq.${cnpjoucpf}&tipo_servico=eq.${encodeURIComponent('31 Autovistoria')}&order=data_cadastro.desc&limit=1`, {
              headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` }
            })
            const ccData = await resCC.json()
